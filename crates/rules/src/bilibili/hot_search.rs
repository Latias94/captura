...
/// 内置 Bilibili 规则示例：热搜（hot-search）。
///
/// 对应 RSSHub 路由 `/bilibili/hot-search`，使用官方 JSON API：
...
pub fn rule() -> RuleSpecV1 {
    ...
    RuleSpecV1 {
        ...
        version: 1,
        description: Some("Bilibili 热搜（JSON API）".to_string()),
        ...
        source: SourceSpec {
            ...
            content: Some(ContentSpec {
                // 对 JSON 源，content 不直接使用；我们将 icon URL 映射到 content_html，
                // 再通过 description_template 生成带图片的描述。
            ...
            mapping: Some(JsonMappingSpec {
                ...
                // 将 icon URL 映射到 content_html，便于在模板中生成 <img>。
                summary: None,
