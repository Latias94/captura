...
pub const META_SPRING_BLOG: RouteMeta = RouteMeta {
    hub_id: "spring/blog",
    path: "/spring/blog",
    ...
    radar: &[Radar {
        source: &["spring.io/blog", "spring.io"],
        target: "/blog",
    }],
    name: "Spring 官方博客",
    maintainers: &["captura"],
    url: "https://spring.io/blog",
    description: "Spring 官方博客（基于 Atom/RSS feed，包含文章内容）。",
    default_view: Some("articles"),
};

pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let limit = ctx.param_i64("limit").unwrap_or(40).max(1) as usize;
    let feed_url = "https://spring.io/blog.atom";
    ...
    let feed = match fetcher.fetch_feed(feed_url).await {
        ...
        Err(e) => {
            return Ok(HubData {
                title: "Spring Blog".to_string(),
                description: Some(format!(
                    "Spring RSS feed is currently unavailable or not a valid XML feed: {}",
                    e
                )),
                link: Some("https://spring.io/blog".to_string()),
                image: Some("https://spring.io/favicon-32x32.png".to_string()),
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "Spring Blog".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://spring.io/blog".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
        .or_else(|| feed.logo.as_ref().map(|l| l.uri.clone()))
        .or_else(|| Some("https://spring.io/favicon-32x32.png".to_string()));
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        let mut categories = entry
            ...
            .map(|c| c.term.clone())
            .collect::<Vec<_>>();
        if !categories.contains(&"spring".to_string()) {
            categories.push("spring".to_string());
        }
    ...
    Ok(HubData {
        title: feed_title,
        description: Some("Spring 官方博客文章列表（基于 Atom/RSS feed）。".to_string()),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_SPRING_BLOG: Route = Route {
    meta: &META_SPRING_BLOG,
    handler: handler_fn,
};
