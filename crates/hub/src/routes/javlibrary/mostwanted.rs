...
pub const META_JAVLIBRARY_MOSTWANTED: RouteMeta = RouteMeta {
    hub_id: "javlibrary/mostwanted",
    path: "/javlibrary/mostwanted/:language?/:mode?",
    ...
    radar: &[Radar {
        ...
        target: "/mostwanted/:language?",
    }],
    name: "JavLibrary 想看最多",
    maintainers: &["captura"],
    url: "https://www.javlibrary.com",
    description: "JavLibrary 想看人数最多榜（最近一月/全时期），基于 vl_mostwanted.php?list&mode= 页面，仅提供元数据。",
...
fn extract_items(html: &str, language: &str, limit: usize) -> captura_common::Result<Vec<HubItem>> {
    ...
    let sel_anchor = Selector::parse(".videotextlist a, #video_comments a").map_err(|e| {
        Error::Parse(format!(
            "javlibrary mostwanted: invalid anchor selector: {e}"
        ))
    })?;
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    Ok(HubData {
        title: format!("JavLibrary most wanted ({})", language),
        description: Some(
            "JavLibrary 想看人数最多作品榜（基于列表页元信息，未抓详情页）。".to_string(),
        ),
        link: Some(url),
        image: None,
...
#[register_hub_route]
pub const ROUTE_JAVLIBRARY_MOSTWANTED: Route = Route {
    meta: &META_JAVLIBRARY_MOSTWANTED,
    handler: handler_fn,
};
