...
pub const META_JAVLIBRARY_GENRE: RouteMeta = RouteMeta {
    hub_id: "javlibrary/genre",
    path: "/javlibrary/genre/:genre?/:language?/:mode?",
...
fn extract_items(html: &str, language: &str, limit: usize) -> captura_common::Result<Vec<HubItem>> {
    ...
    let sel_anchor = Selector::parse(".videotextlist a, #video_comments a")
        .map_err(|e| Error::Parse(format!("javlibrary genre: invalid anchor selector: {e}")))?;
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let genre = ctx.param_str("genre").unwrap_or("amjq");
    ...
    let url = format!(
        "{}/{}/vl_genre.php?list&g={}&mode={}",
        ROOT_URL, language, genre, mode
    );
    ...
    Ok(HubData {
        title: format!("JavLibrary genre {} ({})", genre, language),
        description: Some(
            "JavLibrary 指定类别的作品列表（基于列表页元信息，未抓详情页）。".to_string(),
        ),
        link: Some(url),
        image: None,
...
#[register_hub_route]
pub const ROUTE_JAVLIBRARY_GENRE: Route = Route {
    meta: &META_JAVLIBRARY_GENRE,
    handler: handler_fn,
};
