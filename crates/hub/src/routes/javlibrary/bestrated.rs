...
pub const META_JAVLIBRARY_BESTRATED: RouteMeta = RouteMeta {
    hub_id: "javlibrary/bestrated",
    path: "/javlibrary/bestrated/:language?/:mode?",
    ...
    radar: &[Radar {
        ...
        target: "/bestrated/:language?",
    }],
    name: "JavLibrary 评分最高",
    maintainers: &["captura"],
    url: "https://www.javlibrary.com",
    description: "JavLibrary 最佳评分榜（最近一月/全时期），基于 vl_bestrated.php?list&mode= 页面，仅提供元数据。",
...
fn extract_items(html: &str, language: &str, limit: usize) -> captura_common::Result<Vec<HubItem>> {
    ...
    let sel_anchor = Selector::parse(".videotextlist a, #video_comments a").map_err(|e| {
        Error::Parse(format!(
            "javlibrary bestrated: invalid anchor selector: {e}"
        ))
    })?;
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    Ok(HubData {
        title: format!("JavLibrary best rated ({})", language),
        description: Some(
            "JavLibrary 评分最高作品榜（基于列表页元信息，未抓详情页）。".to_string(),
        ),
        link: Some(url),
        image: None,
...
#[register_hub_route]
pub const ROUTE_JAVLIBRARY_BESTRATED: Route = Route {
    meta: &META_JAVLIBRARY_BESTRATED,
    handler: handler_fn,
};
