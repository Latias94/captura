...
pub const META_GCORES_COLLECTIONS: RouteMeta = RouteMeta {
    hub_id: "gcores/collections",
    path: "/gcores/collections/:id/:tab?",
    categories: &["game"],
    ...
    radar: &[Radar {
        ...
        target: "/gcores/collections/:id/:tab?",
    }],
    name: "机核 - 专题",
    maintainers: &["captura"],
    url: "https://www.gcores.com",
    description: "机核专题聚合内容。",
    default_view: Some("articles"),
};

pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let id = ctx.param_str("id").ok_or_else(|| {
        captura_common::Error::Config("gcores/collections: id is required".to_string())
    })?;
    let tab = ctx.param_str("tab");
    let limit = 30usize;

    let target_url = if let Some(t) = tab {
        format!("{}/collections/{}?tab={}", BASE_URL, id, t)
    } else {
        format!("{}/collections/{}", BASE_URL, id)
    };
    let api_tab = tab.unwrap_or("originals");
    let api_url = format!("{}/gapi/v1/collections/{}/{}", BASE_URL, id, api_tab);
...
#[register_hub_route]
pub const ROUTE_GCORES_COLLECTIONS: Route = Route {
    meta: &META_GCORES_COLLECTIONS,
    handler: handler_fn,
};
