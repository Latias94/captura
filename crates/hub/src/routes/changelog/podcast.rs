...
pub const META_CHANGELOG_PODCAST: RouteMeta = RouteMeta {
    hub_id: "changelog/podcast",
    path: "/changelog/podcast",
    categories: &["programming"],
    example: "/changelog/podcast",
    params: &[ParamMeta {
        name: "limit",
    ...
    radar: &[Radar {
        source: &["changelog.com/podcast"],
        target: "/podcast",
    }],
    name: "The Changelog",
    maintainers: &["captura"],
    url: "https://changelog.com/podcast",
    description: "The Changelog 主播客（软件开发与开源访谈），基于官方 RSS https://changelog.com/podcast/feed，自动内嵌音频。",
    default_view: Some("podcast"),
};

pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let limit = ctx.param_i64("limit").unwrap_or(10).max(1) as usize;
    let feed_url = "https://changelog.com/podcast/feed";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "The Changelog: Software Development, Open Source".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://changelog.com/podcast".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        // 音频 enclosure：优先 content.src，再找 rel=enclosure 的 audio。
    ...
    Ok(HubData {
        title: feed_title,
        description: Some("The Changelog 主播客：软件开发、开源与工程实践访谈。".to_string()),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_CHANGELOG_PODCAST: Route = Route {
    meta: &META_CHANGELOG_PODCAST,
    handler: handler_fn,
};
