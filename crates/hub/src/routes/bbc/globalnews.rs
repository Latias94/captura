...
pub const META_BBC_GLOBALNEWS: RouteMeta = RouteMeta {
    hub_id: "bbc/globalnews",
    path: "/bbc/globalnews",
    categories: &["traditional-media"],
    example: "/bbc/globalnews",
    params: &[ParamMeta {
        name: "limit",
    ...
    radar: &[Radar {
        source: &[
            "www.bbc.co.uk/programmes/p02nq0gn",
            "podcasts.files.bbci.co.uk/p02nq0gn.rss",
        ],
        target: "/globalnews",
    }],
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    let feed_url = "https://podcasts.files.bbci.co.uk/p02nq0gn.rss";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "BBC Global News Podcast".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://www.bbc.co.uk/programmes/p02nq0gn".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        // 播客音频链接：优先 content.src，其次 rel=enclosure 且为 audio。
    ...
    Ok(HubData {
        title: feed_title,
        description: Some(
            "BBC World Service Global News Podcast feed, converted to captura Hub format."
                .to_string(),
        ),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_BBC_GLOBALNEWS: Route = Route {
    meta: &META_BBC_GLOBALNEWS,
    handler: handler_fn,
};
