use crate::routes::types::{Features, HubCtx, HubData, ParamMeta, Radar, Route, RouteMeta};
use captura_common::Error;
use captura_hub_macros::register_hub_route;

use super::{CnbetaKind, ROOT_URL, fetch_cnbeta};

pub const META_CNBETA_TOPICS: RouteMeta = RouteMeta {
    hub_id: "cnbeta/topics",
    path: "/cnbeta/topics/:id",
    categories: &["new-media"],
    example: "/cnbeta/topics/453",
    params: &[ParamMeta {
        name: "id",
        description: "Topic id from the URL, see https://www.cnbeta.com.tw/topics.htm for full list.",
        default: None,
        options: &[],
    }],
    features: Features::with_anti_crawler(),
    radar: &[Radar {
        source: &["cnbeta.com.tw/topics/:id"],
        target: "/topics/:id",
    }],
    name: "cnBeta 主题",
    maintainers: &["captura"],
    url: "https://www.cnbeta.com.tw",
    description: "cnBeta.COM topic streams, aligned with RSSHub /cnbeta/topics/:id route.",
    default_view: Some("articles"),
};

pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let id = ctx
        .param_str("id")
        .ok_or_else(|| Error::Config("cnbeta/topics: missing id parameter".to_string()))?;
    let limit = ctx.param_i64("limit").unwrap_or(60).max(1) as usize;

    let (items, title, description) =
        fetch_cnbeta(CnbetaKind::Topics { id: id.to_string() }, limit).await?;

    Ok(HubData {
        title,
        description,
        link: Some(format!("{}/topics/{}.htm", ROOT_URL, id)),
        image: None,
        language: Some("zh-TW".to_string()),
        items,
        allow_empty: true,
    })
}

fn handler_fn<'a>(ctx: &'a mut HubCtx<'a>) -> crate::routes::types::HubHandlerFuture<'a> {
    Box::pin(handler(ctx))
}

#[register_hub_route]
pub const ROUTE_CNBETA_TOPICS: Route = Route {
    meta: &META_CNBETA_TOPICS,
    handler: handler_fn,
};
