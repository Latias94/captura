...
pub const META_REACT_BLOG: RouteMeta = RouteMeta {
    hub_id: "react/blog",
    path: "/react/blog",
    ...
    radar: &[Radar {
        source: &["react.dev/blog", "react.dev"],
        target: "/blog",
    }],
    name: "React 官方博客",
    maintainers: &["captura"],
    url: "https://react.dev/blog",
    description: "React 官方博客（基于 React Blog RSS feed）。",
    default_view: Some("articles"),
};

pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let limit = ctx.param_i64("limit").unwrap_or(30).max(1) as usize;
    let feed_url = "https://react.dev/rss.xml";
    ...
    let feed = match fetcher.fetch_feed(feed_url).await {
        ...
        Err(e) => {
            return Ok(HubData {
                title: "React Blog".to_string(),
                description: Some(format!(
                    "React RSS feed is currently unavailable or not a valid XML feed: {}",
                    e
                )),
                link: Some("https://react.dev/blog".to_string()),
                image: Some("https://react.dev/images/og-blog.png".to_string()),
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "React Blog".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://react.dev/blog".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
        .or_else(|| feed.logo.as_ref().map(|l| l.uri.clone()))
        .or_else(|| Some("https://react.dev/images/og-blog.png".to_string()));
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        let mut categories = entry
            ...
            .map(|c| c.term.clone())
            .collect::<Vec<_>>();
        if !categories.contains(&"react".to_string()) {
    ...
    Ok(HubData {
        title: feed_title,
        description: Some("React 官方博客（基于 RSS feed，包含摘要内容）。".to_string()),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_REACT_BLOG: Route = Route {
    meta: &META_REACT_BLOG,
    handler: handler_fn,
};
