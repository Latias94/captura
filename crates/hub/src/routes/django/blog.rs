...
pub const META_DJANGO_BLOG: RouteMeta = RouteMeta {
    hub_id: "django/blog",
    path: "/django/blog",
    ...
    radar: &[Radar {
        source: &["www.djangoproject.com/weblog", "djangoproject.com/weblog"],
        target: "/weblog",
    }],
    name: "Django 官方博客",
    maintainers: &["captura"],
    url: "https://www.djangoproject.com/weblog/",
    description: "Django 官方博客（The Django weblog），基于 RSS feed，包含文章内容。",
    default_view: Some("articles"),
};

pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let limit = ctx.param_i64("limit").unwrap_or(30).max(1) as usize;
    let feed_url = "https://www.djangoproject.com/rss/weblog/";
    ...
    let feed = match fetcher.fetch_feed(feed_url).await {
        ...
        Err(e) => {
            return Ok(HubData {
                title: "The Django weblog".to_string(),
                description: Some(format!(
                    "Django RSS feed is currently unavailable or not a valid XML feed: {}",
                    e
                )),
                link: Some("https://www.djangoproject.com/weblog/".to_string()),
                image: Some(
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "The Django weblog".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://www.djangoproject.com/weblog/".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
        ...
        .or_else(|| {
            Some("https://static.djangoproject.com/img/logos/django-logo-negative.png".to_string())
        });
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        let mut categories = entry
            ...
            .map(|c| c.term.clone())
            .collect::<Vec<_>>();
        if !categories.contains(&"django".to_string()) {
    ...
    Ok(HubData {
        title: feed_title,
        description: Some("Django 官方博客文章列表（基于 RSS feed）。".to_string()),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_DJANGO_BLOG: Route = Route {
    meta: &META_DJANGO_BLOG,
    handler: handler_fn,
};
