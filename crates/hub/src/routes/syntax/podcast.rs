...
pub const META_SYNTAX_PODCAST: RouteMeta = RouteMeta {
    hub_id: "syntax/podcast",
    path: "/syntax/podcast",
    categories: &["programming"],
    example: "/syntax/podcast",
    params: &[ParamMeta {
        name: "limit",
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    let feed_url = "https://feed.syntax.fm/rss";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "Syntax - Tasty Web Development Treats".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://syntax.fm".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        if !categories
            .iter()
            .any(|c| c.eq_ignore_ascii_case("javascript") || c.eq_ignore_ascii_case("web"))
        {
    ...
    Ok(HubData {
        title: feed_title,
        description: Some("Syntax：前端与全栈 Web 开发主题播客。".to_string()),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_SYNTAX_PODCAST: Route = Route {
    meta: &META_SYNTAX_PODCAST,
    handler: handler_fn,
};
