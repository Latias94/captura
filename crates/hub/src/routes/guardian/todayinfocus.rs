...
pub const META_GUARDIAN_TODAYINFOCUS: RouteMeta = RouteMeta {
    hub_id: "guardian/todayinfocus",
    path: "/guardian/todayinfocus",
    categories: &["traditional-media"],
    example: "/guardian/todayinfocus",
    params: &[ParamMeta {
        name: "limit",
    ...
    radar: &[Radar {
        source: &["www.theguardian.com/news/series/todayinfocus"],
        target: "/todayinfocus",
    }],
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    let feed_url = "https://www.theguardian.com/news/series/todayinfocus/podcast.xml";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "Today in Focus".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://www.theguardian.com/news/series/todayinfocus".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        // 音频链接：使用 content.src 或 rel=enclosure 的音频链接
    ...
    Ok(HubData {
        title: feed_title,
        description: Some(
            "Today in Focus – Guardian daily deep-dive news podcast feed via official XML."
                .to_string(),
        ),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_GUARDIAN_TODAYINFOCUS: Route = Route {
    meta: &META_GUARDIAN_TODAYINFOCUS,
    handler: handler_fn,
};
