...
pub const META_GUARDIAN_RSS: RouteMeta = RouteMeta {
    hub_id: "guardian/rss",
    path: "/guardian/rss/:section?",
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    let section = ctx.param_str("section").unwrap_or("world");
    let section = if section.is_empty() { "world" } else { section };
    let feed_url = format!("https://www.theguardian.com/{}/rss", section);
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| format!("The Guardian - {}", section));
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| format!("https://www.theguardian.com/{}", section));
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    Ok(HubData {
        title: feed_title,
        description: Some(format!("The Guardian RSS section: {}", section)),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_GUARDIAN_RSS: Route = Route {
    meta: &META_GUARDIAN_RSS,
    handler: handler_fn,
};
