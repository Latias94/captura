...
pub const META_NYTIMES_DAILY: RouteMeta = RouteMeta {
    hub_id: "nytimes/daily",
    path: "/nytimes/daily",
    categories: &["traditional-media"],
    example: "/nytimes/daily",
    params: &[ParamMeta {
        name: "limit",
    ...
    radar: &[Radar {
        source: &["www.nytimes.com/the-daily", "feeds.simplecast.com/54nAGcIl"],
        target: "/daily",
    }],
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    let feed_url = "https://feeds.simplecast.com/54nAGcIl";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "The Daily".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://www.nytimes.com/the-daily".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
        ...
        .or_else(|| {
            Some("https://image.simplecastcdn.com/images/7f2f4c05-9c2f-4deb-82b7-b538062bc22d/73549bf1-94b3-40ff-8aeb-b4054848ec1b/3000x3000/the-daily-album-art-original.jpg?aid=rss_feed".to_string())
        });
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        // 音频 enclosure：优先使用 content.src，其次 links 中 rel=enclosure 且 media_type 为 audio。
        ...
        if audio_url.is_none() {
            if let Some(enclosure_link) = entry.links.iter().find(|l| {
                ...
                audio_url = Some(enclosure_link.href.clone());
            }
        }
    ...
    Ok(HubData {
        title: feed_title,
        description: Some(
            "This is what the news should sound like — New York Times The Daily podcast feed."
                .to_string(),
        ),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_NYTIMES_DAILY: Route = Route {
    meta: &META_NYTIMES_DAILY,
    handler: handler_fn,
};
