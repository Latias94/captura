...
pub const META_SEDAILY_PODCAST: RouteMeta = RouteMeta {
    hub_id: "softwareengineeringdaily/podcast",
    path: "/softwareengineeringdaily/podcast",
    categories: &["programming"],
    example: "/softwareengineeringdaily/podcast",
    params: &[ParamMeta {
        name: "limit",
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    let feed_url = "https://softwareengineeringdaily.com/feed/podcast/";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "Software Engineering Daily".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://softwareengineeringdaily.com/".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    Ok(HubData {
        title: feed_title,
        description: Some(
            "Software Engineering Daily：软件工程与架构相关的技术访谈播客。".to_string(),
        ),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_SEDAILY_PODCAST: Route = Route {
    meta: &META_SEDAILY_PODCAST,
    handler: handler_fn,
};
