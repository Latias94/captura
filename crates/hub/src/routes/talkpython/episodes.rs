...
pub const META_TALKPYTHON_EPISODES: RouteMeta = RouteMeta {
    hub_id: "talkpython/episodes",
    path: "/talkpython/episodes",
    categories: &["programming"],
    example: "/talkpython/episodes",
    params: &[ParamMeta {
        name: "limit",
    ...
    radar: &[Radar {
        source: &["talkpython.fm/episodes/show/*", "talkpython.fm/episodes"],
        target: "/episodes",
    }],
...
pub async fn handler(ctx: &mut HubCtx<'_>) -> captura_common::Result<HubData> {
    ...
    let feed_url = "https://talkpython.fm/episodes/rss";
    ...
    let feed_title = feed
        ...
        .unwrap_or_else(|| "Talk Python To Me".to_string());
    let feed_link = feed
        .links
        .get(0)
        .map(|l| l.href.clone())
        .unwrap_or_else(|| "https://talkpython.fm/episodes".to_string());
    let feed_image = feed
        .icon
        .as_ref()
        .map(|i| i.uri.clone())
    ...
    for entry in feed.entries.into_iter().take(limit) {
        ...
        if !categories.iter().any(|c| c.eq_ignore_ascii_case("python")) {
    ...
    Ok(HubData {
        title: feed_title,
        description: Some("Talk Python To Me：Python 社区技术访谈播客。".to_string()),
        link: Some(feed_link),
        image: feed_image,
...
#[register_hub_route]
pub const ROUTE_TALKPYTHON_EPISODES: Route = Route {
    meta: &META_TALKPYTHON_EPISODES,
    handler: handler_fn,
};
